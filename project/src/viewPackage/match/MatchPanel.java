...
public class MatchPanel extends JPanel implements Panel {
    ...
    public void enterPanel() {
        ...
        String[] columnNames = {"ID", "Team Blue Name", "Team Red Name", "Competition Name", "Occurrence Date", "Winner Team", "Replay URL", "Summary"};
        ...
        for (int i = 0; i < this.allMatches.size(); i++) {
            ...
            data[i][5] = match.isBlueWin() ? match.getTeamBlue().getName() : match.getTeamRed().getName();
            data[i][6] = match.getReplayLink() != null ? match.getReplayLink() : "N/A";
            data[i][7] = match.getSummary() != null ? match.getSummary() : "N/A";
        }
        matchTable.setModel(new DefaultTableModel(data, columnNames));
